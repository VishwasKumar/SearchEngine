...
public class SearchEngineTester {
	...
	public static void searchKeyword(String keyword) throws Exception {
        ...
        for (KeyValue kv : indexRow.list()) {
			...
			Get getData = new Get(Bytes.toBytes(pageDocId));
			...
			Get getPageRank = new Get(Bytes.toBytes(pageDocId));
			Result pageRankRow = prTable.get(getPageRank);
			pageRank = Bytes.toFloat(pageRankRow.raw()[0].getQualifier());
